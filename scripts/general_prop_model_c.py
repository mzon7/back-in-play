#!/usr/bin/env python3
"""
General Prop Model C — EWA-based features with minutes-rate decomposition.
...
Feature Architecture (~17 features):
...
Uses LightGBM with CV-based feature pruning (Model E style).

Usage:
  python3 general_prop_model_c.py --league nba
"""
...
def load_all_data(league):
    ...
    # This maps to all game logs where a player faced that opponent on that date.
    # Useful for computing team totals and opponent-allowed stats.
...
def build_dataset(data, league):
    """Build dataset for ALL players (no injury filter) with EWA features."""
    props = data["props"]
    ...
    # We'll compute these on the fly per-market to avoid loading everything into memory.
    # Instead, cache as we go.
    _league_avg_cache = {}
    ...
    def get_league_avg_stat(stat_key_or_combo, market, all_logs_by_opp_date):
        ...
        # Collect per-date team totals and average them
        # Group by (team's opponent code, date) -> sum of stat
        vals = []
        sampled = 0
    ...
    for prop in props:
        ...
        # Compute per-game minute shares
        minute_shares = []
        for g in prior_games[-10:]:
            ...
            gd = g.get("game_date")
            if opp_code and gd:
                # Team = all players who had the same opponent on the same date
        ...
        # Compute per-minute rate for each prior game
        per_min_rates = []
        for g in prior_games:
            if not g.get("minutes") or g["minutes"] <= 0:
                continue
        ...
        # 9. projected_stat_opp_adj — need opp_defense_ratio first
        # (computed in section D below, then back-filled)
        ...
        # 10. team_stat_form: EWA(team total for stat, last 5) / league_avg
        # The player's team = players sharing the same opponent code on a given date
        # We use prior_games to get the team's recent game dates + opponent codes
        team_totals = []
        ...
        # 11. opp_defense_ratio: EWA(opponent allowed for stat, last 10) / league_avg
        # "Opponent allowed" = what other teams scored against this opponent.
        # Find games where other players had opponent = player_opp_code (i.e. games vs this opponent)
        # Collect unique dates where teams played vs this opponent
        opp_allowed_totals = []
        opp_game_dates = set()
        ...
        # 12. game_pace_factor: game_total_line / league_avg_total
        ...
        # 13. team_implied_edge: team_implied_points / league_avg_team_points
...
def run(league):
    print(f"\n{'=' * 60}")
    print(f"GENERAL PROP MODEL C — {league.upper()} (Walk-Forward)")
    print(f"{'=' * 60}\n")
    ...
    for train_book in ["fanduel", "draftkings"]:
        ...
        print("\nBuilding dataset (ALL players, EWA features)...")
        ...
        for wi, window in enumerate(windows):
            ...
            window_bets = make_bets(window_test_all, y_proba, "general_c", season)
        ...
        league_key = f"{league}_general_c_{book_tag}"
        results = {
            "league": league, "model": "general_c",
        ...
        out = f"/workspace/back-in-play/data/general_c_{league}_{book_tag}.json"
