    ...
    queryFn: async () => {
      ...
      for (const [pid, inj] of byPlayer) {
        ...
        result.push({
          ...
          expected_return: inj.expected_return, status: inj.status,
...
function PlayerCard({ player, multiLeague }: { player: ReturningPlayer; multiLeague: boolean }) {
  ...
  const g1 = medianPct[0] != null ? Math.round((medianPct[0] as number) * 100) : null;
        ...
        <div className="px-4 pb-3">
          <div className="rounded-lg bg-white/[0.03] border border-white/8 p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] text-white/40 uppercase tracking-wider">Expected G1 Performance</span>
              <span className={`text-sm font-bold ${g1 >= 95 ? "text-green-400" : g1 >= 80 ? "text-amber-400" : "text-red-400"}`}>
                {g1}% of baseline
              </span>
            </div>
            {stats.length > 0 && statAvg && (
              <div className="flex flex-wrap gap-2 mt-1">
                {stats.slice(0, 4).map((s) => {
                  ...
                  const v = arr[0] != null ? Math.round((arr[0] as number) * 100) : null;
