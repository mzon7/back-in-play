#!/usr/bin/env python3
"""
General Prop Model F2 — EWA-based features with expanded feature set + CV pruning.

Built on Model C's architecture. Adds ~10 new features (season baseline, pace,
matchup interactions, variance, team over-rate, position matchup) then uses
5-fold CV permutation-importance pruning to drop features with mean importance < 0.001.

Feature Architecture (~27 features):
  A. Expected Minutes/Role (4): expected_minutes, minute_share, minutes_trend, role_stability
  B. Per-Minute Production (3): blended_per_min_rate, per_min_trend, per_min_stability
  C. Projected Stat (2): projected_stat, projected_stat_opp_adj
  D. Team & Opponent Context (4): team_stat_form, opp_defense_ratio, game_pace_factor, team_implied_edge
  E. Market & Context (4): line_vs_projection, open_implied_prob, is_home, rest_days
  F. New F2 Features (10): season_per_min_rate, games_played, team_pace_rank, opp_pace_rank,
     matchup_pace_factor, minutes_x_opp_defense, projected_vs_line, player_variance,
     team_over_rate, position_matchup

Uses LightGBM with 5-fold CV permutation-importance pruning (drop features < 0.001).

Usage:
  python3 general_prop_model_f2.py --league nba
"""
...
FEATURES = [
    ...
    "blended_per_min_rate",     # w * EWA(stat/min, last 10) + (1-w) * season_per_min_rate
    "per_min_trend",            # EWA(stat/min, last 3) / season_per_min_rate
    "per_min_stability",        # 1 - CV(stat/min over last 10), clipped [0,1]
    # C. Projected Stat
    ...
    "game_pace_factor",         # game_total_line / league_avg_total
    "team_implied_edge",        # team_implied_points / league_avg_team_points
    # E. Market & Context
    "line_vs_projection",       # prop_line / projected_stat
    "open_implied_prob",        # market implied probability
    "is_home",                  # 1/0/0.5
    ...
    # G. New F2 Features
    "season_per_min_rate",      # player's full season stat/min rate (baseline reference)
    "games_played",             # number of games the player has appeared in this season
    "team_pace_rank",           # team's pace relative to league (higher = more possessions)
    "opp_pace_rank",            # opponent's pace relative to league
    "matchup_pace_factor",      # team_pace_rank * opp_pace_rank
    "minutes_x_opp_defense",   # expected_minutes * opp_defense_ratio (interaction)
    "projected_vs_line",        # projected_stat / prop_line (ratio, not difference)
    "player_variance",          # CV of the stat over last 10 games (high variance = harder to predict)
    "team_over_rate",           # fraction of times this team's players go over their lines recently
    "position_matchup",         # interaction between position and opponent defense ranking
]
...
# ─── Position encoding for position_matchup feature ─────────────────────────
POSITION_ENCODING = {
    # NBA
    ...
    "FW": 1, "MF": 2, "DF": 3, "GK": 4,
    # MLB
    "SP": 1, "RP": 2, "DH": 3, "1B": 4, "2B": 5, "3B": 6, "SS": 7,
    "LF": 8, "CF": 9, "RF": 10,
}
...
def load_all_data(league):
    ...
    for p in players_raw:
        ...
        player_team_id[p["player_id"]] = p.get("team_id")
...
def build_dataset(data, league):
    """Build dataset for ALL players with EWA features + F2 expanded features."""
    props = data["props"]
    ...
    home_games_set = set()
    for go in game_odds.values():
        ht = (go.get("home_team") or "").lower()
        gd = go.get("game_date")
        if ht and gd:
            home_games_set.add((ht, gd))

    # Pre-compute league average total from game odds (for game_pace_factor)
    all_totals = []
    all_team_implied = []
    for go in game_odds.values():
        total = float(go.get("total_line") or 0)
        if total > 0:
            all_totals.append(total)
            spread = float(go.get("spread_home_line") or 0)
            all_team_implied.append((total - spread) / 2)
            all_team_implied.append((total + spread) / 2)
    league_avg_total = statistics.mean(all_totals) if all_totals else 200.0
    league_avg_team_pts = statistics.mean(all_team_implied) if all_team_implied else 100.0

    # ─── Pre-compute team pace ranks from game odds ──────────────────────
    # For each team, compute average game total when they play -> pace proxy
    # Then rank them: pace_rank = team_avg_total / league_avg_total
    _team_game_totals = defaultdict(list)
    for go in game_odds.values():
        total = float(go.get("total_line") or 0)
        if total <= 0:
            continue
        ht = (go.get("home_team") or "").lower()
        at = (go.get("away_team") or "").lower()
        if ht:
            _team_game_totals[ht].append(total)
        if at:
            _team_game_totals[at].append(total)
    team_pace_map = {}
    for team, totals in _team_game_totals.items():
        if totals:
            team_pace_map[team] = statistics.mean(totals) / league_avg_total if league_avg_total > 0 else 1.0
    # Default pace rank = 1.0 (league average)

    # ─── Pre-compute team over-rate tracking ─────────────────────────────
    # We'll build this as we go through props, but pre-index by team+market
    # For simplicity, compute from the props themselves: for each team, what
    # fraction of their players' props went over recently.
    # We'll build a lookup: (team_name_lower, market) -> list of (date, went_over)
    # Then at prediction time, look back at the last N entries before game_date.

    samples = []
    skips = defaultdict(int)

    # Pre-compute league averages per stat from all game logs by opp_date
    _league_avg_cache = {}
    ...
    # ─── Build team over-rate index (pre-pass through props) ─────────────
    # Sort props by date, then for each team+market track actual > line
    _team_over_index = defaultdict(list)  # (team_lower, market) -> [(date, went_over)]
    # We need actual results, so we'll populate this during the main loop
    # and look back. To avoid look-ahead, we process chronologically.
    props_sorted = sorted(props, key=lambda p: p.get("game_date", ""))
    ...
    for prop in props_sorted:
        ...
        # 7. per_min_stability
        per_min_stability = max(0.0, min(1.0, 1.0 - coeff_of_variation(last_10_pmr))) if len(last_10_pmr) >= 2 else 0.5
        ...
        # 9. projected_stat_opp_adj — need opp_defense_ratio first (section D)
        ...
        # Now compute 9. projected_stat_opp_adj
        projected_stat_opp_adj = projected_stat * opp_defense_ratio

        # 12. game_pace_factor
        go = game_odds.get(event_id)
        game_total_line = 0.0
        team_spread = 0.0
        team_implied_win_prob = 0.5
        if go:
            game_total_line = float(go.get("total_line") or 0)
            go_home = (go.get("home_team") or "").lower()
            go_away = (go.get("away_team") or "").lower()
            player_team_name = player_team.get(pid, "").lower()
            if player_team_name and player_team_name in go_home:
                team_spread = float(go.get("spread_home_line") or 0)
            elif player_team_name and player_team_name in go_away:
                team_spread = float(go.get("spread_away_line") or 0)
        else:
            player_team_name = player_team.get(pid, "").lower()

        game_pace_factor = game_total_line / league_avg_total if game_total_line > 0 and league_avg_total > 0 else 1.0

        # 13. team_implied_edge
        team_implied_points = (game_total_line - team_spread) / 2 if game_total_line > 0 else 0
        team_implied_edge = team_implied_points / league_avg_team_pts if team_implied_points > 0 and league_avg_team_pts > 0 else 1.0

        # ─── E. Market & Context ─────────────────────────────────────────

        # 14. line_vs_projection
        line_vs_projection = line / projected_stat if projected_stat > 0 else 1.0

        # 15. open_implied_prob
        ...
        open_implied_prob = odds_to_implied_prob(open_over_price) if open_over_price else odds_to_implied_prob(over_price)
        ...
        # 17. rest_days
        prev_game_date = prior_games[-1]["game_date"] if prior_games else game_date
        rest_days = date_diff(prev_game_date, game_date)

        # 18-19. Home/Away splits
        # Filter prior games by matching venue type
        ...
        # 18. season_per_min_rate (already computed above)
        # Use 0 default for leagues without minutes
        f2_season_per_min_rate = season_per_min_rate

        # 19. games_played — number of games this season
        f2_games_played = len(season_games)

        # 20. team_pace_rank — team's pace relative to league
        f2_team_pace_rank = team_pace_map.get(player_team_name_lc, 1.0) if player_team_name_lc else 1.0

        # 21. opp_pace_rank — opponent's pace relative to league
        # Determine opponent team name from game odds
        opp_team_name_lc = ""
        if go:
            go_home = (go.get("home_team") or "").lower()
            go_away = (go.get("away_team") or "").lower()
            if player_team_name_lc and player_team_name_lc in go_home:
                opp_team_name_lc = go_away
            elif player_team_name_lc and player_team_name_lc in go_away:
                opp_team_name_lc = go_home
        f2_opp_pace_rank = team_pace_map.get(opp_team_name_lc, 1.0) if opp_team_name_lc else 1.0

        # 22. matchup_pace_factor — team_pace_rank * opp_pace_rank
        f2_matchup_pace_factor = f2_team_pace_rank * f2_opp_pace_rank

        # 23. minutes_x_opp_defense — expected_minutes * opp_defense_ratio (interaction)
        f2_minutes_x_opp_defense = expected_minutes * opp_defense_ratio

        # 24. projected_vs_line — projected_stat / prop_line (ratio, not difference)
        f2_projected_vs_line = projected_stat / line if line > 0 else 1.0

        # 25. player_variance — CV of the stat over last 10 games
        last_10_stats = prior_stat_values[-10:]
        f2_player_variance = coeff_of_variation(last_10_stats) if len(last_10_stats) >= 2 else 0.0

        # 26. team_over_rate — fraction of recent team props that went over
        # Look back at the last 50 entries for this team+market before this date
        team_over_key = (player_team_name_lc, market)
        recent_team_overs = _team_over_index.get(team_over_key, [])
        # Only use entries before this game_date
        relevant_overs = [went_over for (d, went_over) in recent_team_overs if d < game_date]
        relevant_overs = relevant_overs[-50:]  # last 50
        f2_team_over_rate = statistics.mean(relevant_overs) if relevant_overs else 0.5

        # 27. position_matchup — interaction: position_encoding * opp_defense_ratio
        player_pos = name_to_pos.get(player_name.lower(), "")
        pos_code = POSITION_ENCODING.get(player_pos.upper(), 0) if player_pos else 0
        f2_position_matchup = pos_code * opp_defense_ratio

        # ─── Target ──────────────────────────────────────────────────────
        target = 1 if actual_stat > open_line_val else 0

        # ─── Update team_over_index for future samples ───────────────────
        went_over = 1.0 if actual_stat > line else 0.0
        _team_over_index[team_over_key].append((game_date, went_over))
        ...
        feature_vec = [
            ...
            # G. New F2 Features
            f2_season_per_min_rate,
            f2_games_played,
            f2_team_pace_rank,
            f2_opp_pace_rank,
            f2_matchup_pace_factor,
            f2_minutes_x_opp_defense,
            f2_projected_vs_line,
            f2_player_variance,
            f2_team_over_rate,
            f2_position_matchup,
        ]
...
def run(league):
    print(f"\n{'=' * 60}")
    print(f"GENERAL PROP MODEL F2 — {league.upper()} (Walk-Forward)")
    print(f"{'=' * 60}\n")
    ...
    for train_book in ["fanduel", "draftkings"]:
        ...
        print("\nBuilding dataset (ALL players, EWA + F2 features)...")
        ...
        for wi, window in enumerate(windows):
            ...
            # Per-market training with 5-fold CV permutation-importance pruning
            ...
            for mkt in all_markets:
                ...
                # Train on each fold, compute permutation importance on the OOF,
                # average across folds, drop features with mean importance < 0.001.
                ...
                if n_splits >= 2 and len(mkt_train) >= n_splits * 2:
                    ...
                    perm_importances = np.zeros(len(FEATURES))
                    n_folds_done = 0
                    try:
                        for fold_train_idx, fold_val_idx in cv.split(X_train_full, y_tr):
                            ...
                            # Permutation importance on the OOF validation set
                            perm_result = permutation_importance(
                                fold_model,
                                X_train_full[fold_val_idx],
                                y_tr[fold_val_idx],
                                n_repeats=5,
                                random_state=42,
                                scoring="accuracy",
                            ...
                            n_folds_done += 1
                        if n_folds_done > 0:
                            perm_importances /= n_folds_done
                        # Keep features with mean permutation importance >= 0.001
                        keep_mask = perm_importances >= 0.001
                        # Always keep at least 5 features (safety net)
                        if keep_mask.sum() < 5:
                            top_5_idx = np.argsort(perm_importances)[-5:]
                            keep_mask = np.zeros(len(FEATURES), dtype=bool)
                            keep_mask[top_5_idx] = True
                    except Exception:
                ...
                print(f"    {mkt}: train={len(mkt_train):,} test={len(mkt_test):,} "
                      f"acc={mkt_acc:.4f} auc={mkt_auc:.4f} kept={len(kept_features)}/{len(FEATURES)} (dropped {dropped})")
            ...
            window_bets = make_bets(window_test_all, y_proba, "general_f2", season)
        ...
        league_key = f"{league}_general_f2_{book_tag}"
        results = {
            "league": league, "model": "general_f2",
        ...
        out = f"/workspace/back-in-play/data/general_f2_{league}_{book_tag}.json"
