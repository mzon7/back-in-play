#!/usr/bin/env python3
"""
Team Model F2: Team-level betting predictions (spreads, totals, team totals).

Based on Model C architecture with additional features and 5-fold CV permutation
importance pruning (the key F2 innovation: features with importance < 0.001 are dropped).

Features (~38):
  A. Team Recent Form    (3): team_stat_ewa, team_stat_trend, team_vs_league
  B. Opponent Context    (3): opp_allowed_ewa, opp_allowed_trend, opp_vs_league
  C. Lineup Projection   (3): lineup_projected_stat, lineup_proj_last_game_mins, lineup_proj_10g_mins
  D. Combined            (2): blended_team_projection, opp_adjusted_projection
  E. Market & Context    (4): line_vs_projection, open_implied_prob, is_home, rest_days
  F. League Baseline     (1): league_avg_stat
  G. F2 Extra Features  (10): home_vs_away_spread, h2h_recent_record, team_scoring_streak,
                               opp_scoring_streak, team_rest_advantage, team_season_win_pct,
                               opp_season_win_pct, lineup_completeness, team_stat_variance,
                               projected_vs_team_ewa
  H. Starting Pitcher    (6): sp_k_per_ip_ewa, sp_ip_ewa, sp_season_k_per_ip,
                               sp_ip_consistency, sp_vs_league, sp_recent_form
  I. Bullpen             (3): bp_quality, bp_fatigue, bp_expected_ip
  J. Combined Pitching   (1): pitching_quality_index
  K. Park Factors        (2): park_runs_factor, park_factor_vs_league

Usage:
  python3 team_model_f2.py --league nba
"""
...
# ---------------------------------------------------------------------------
# Shared utilities from regression_v5_model_e.py
# ---------------------------------------------------------------------------
...
from regression_v5_model_e import (
    ...
    make_bets, WALK_FORWARD_WINDOWS,
)
...
# Built from the observed abbreviations in the data
TEAM_ABBR_MAP = {
    # NBA
    "Atlanta Hawks": ["ATL"],
    "Boston Celtics": ["BOS"],
...
FEATURES = [
    ...
    "opp_vs_league",
    # C. Lineup Projection
    "lineup_projected_stat",
    "lineup_proj_last_game_mins",
    "lineup_proj_10g_mins",
    # D. Combined
    "blended_team_projection",
    "opp_adjusted_projection",
    # E. Market & Context
    "line_vs_projection",
    "open_implied_prob",
    "is_home",
    ...
    # F. League Baseline
    "league_avg_stat",
    # F2. Home/Away Splits (all leagues)
    "team_home_away_ewa",
    "team_home_away_diff",
    # G. F2 Extra Features
    "home_vs_away_spread",
    ...
    "team_stat_variance",
    "projected_vs_team_ewa",
    # H. Starting Pitcher (MLB only, 0 for other leagues)
    "sp_k_per_ip_ewa",
    "sp_ip_ewa",
    "sp_season_k_per_ip",
    "sp_ip_consistency",
...
# Permutation importance threshold for 5-fold CV pruning (key F2 innovation)
PERM_IMPORTANCE_THRESHOLD = 0.001
...
def _scoring_streak(games):
    """Compute signed scoring streak: consecutive games above (+) or below (-) the
    team's average scored. Returns 0 if no games."""
    if not games:
        return 0
    avg = statistics.mean([g["scored"] for g in games])
    streak = 0
    ...
    for g in reversed(games):
        if g["scored"] > avg:
            ...
            break  # exactly average — streak broken
    return streak


def _win_pct(team_name_lower, team_scores_dict, team_date_scored_lookup, before_date):
    """Compute season win percentage for a team up to before_date.
    A 'win' is scored > opponent scored on same date."""
    games = team_scores_dict.get(team_name_lower, [])
    past = [g for g in games if g["game_date"] < before_date]
    if not past:
        return 0.5  # default
    wins = 0
    total = 0
    for g in past:
        ...
        if not opp:
            continue
        opp_scored = team_date_scored_lookup.get((opp, g["game_date"]))
        if opp_scored is None:
            continue
        total += 1
        if g["scored"] > opp_scored:
            wins += 1
...
def load_teams_and_players(league):
    ...
    for tname, abbrs in TEAM_ABBR_MAP.items():
        for a in abbrs:
            # Only map if this team name actually exists in our DB for this league
...
def compute_team_features(
    ...
    league_games_before, stat_col, team_date_scored_lookup,
    league=None, player_position=None, starter_cache=None, home_games_set=None,
):
    """Compute the 40 features for a single team in a single game (28 base + 10 pitcher + 2 park).

    Returns feature vector (list of 26 floats) or None if insufficient data.
    """
    ...
    team_stat_ewa_val = ewa_shrunk(last_10, season_avg)
    ewa_last3 = ewa(last_3) if last_3 else team_stat_ewa_val
    ewa_last10 = ewa(last_10) if last_10 else team_stat_ewa_val
    ...
    # Track expected minutes for lineup_completeness
    total_prev_minutes = 0.0
    covered_minutes = 0.0

    if not prev_player_logs:
        # Can't build lineup projection
        lineup_projected_stat = team_stat_ewa_val  # fallback
        lineup_proj_last_game_mins = team_stat_ewa_val
        lineup_proj_10g_mins = team_stat_ewa_val
        lineup_completeness_val = 0.0
    else:
        lineup_proj_sum = 0.0
        lineup_proj_lgm = 0.0
        lineup_proj_10g = 0.0
        ...
        for plog in prev_player_logs:
            ...
            prev_mins = plog.get("minutes", 0) or 0
            total_prev_minutes += prev_mins
            ...
            # This player is available (has history) — count their expected minutes as covered
            covered_minutes += prev_mins
            ...
            # Expected minutes variants
            last_5_mins = p_mins[-5:]
            ewa_mins_last5 = ewa(last_5_mins) if last_5_mins else 0
            last_game_mins = p_mins[-1] if p_mins else 0
            avg_mins_last10 = statistics.mean(p_mins[-10:]) if p_mins else 0

            lineup_proj_sum += blended_per_min * ewa_mins_last5
            lineup_proj_lgm += blended_per_min * last_game_mins
            lineup_proj_10g += blended_per_min * avg_mins_last10

        lineup_projected_stat = lineup_proj_sum if lineup_proj_sum > 0 else team_stat_ewa_val
        lineup_proj_last_game_mins = lineup_proj_lgm if lineup_proj_lgm > 0 else team_stat_ewa_val
        lineup_proj_10g_mins = lineup_proj_10g if lineup_proj_10g > 0 else team_stat_ewa_val
        lineup_completeness_val = covered_minutes / total_prev_minutes if total_prev_minutes > 0 else 0.0

    # --- D. Combined Projection ---
    blended_team_projection = (
        0.5 * lineup_projected_stat +
        0.3 * team_stat_ewa_val +
        0.2 * league_avg
    )
    opp_adjusted_projection = blended_team_projection * opp_vs_league_val

    # --- E. Market & Context ---
    line_vs_projection = market_line / opp_adjusted_projection if opp_adjusted_projection > 0 else 1.0
    open_implied_prob = odds_to_implied_prob(market_price_over) if market_price_over is not None else 0.5
    is_home = 1.0 if is_home_flag else 0.0

    # Rest days: days since team's last game
    rest = date_diff(prev_game_date, game_date)
    rest_days = min(rest, 14)  # cap at 14
    ...
    if home_games_set:
        ...
        if is_home_flag and len(home_scored) >= 3:
            ...
            team_home_away_ewa = team_stat_ewa_val  # fallback to overall
        ...
        team_home_away_ewa = team_stat_ewa_val
        team_home_away_diff = 0.0
    ...
    # --- home_vs_away_spread: historical home/away performance difference ---
    # Compute team's avg scored at home vs away from past games
    home_scored_vals = []
    away_scored_vals = []
    for g in past_games:
        # Determine if team was home in that past game by checking team_date_scored
        # If the team appears as home (i.e., the opponent had an away game on that date),
        # we approximate: if the game appears in team_game_players with the team as the
        # first key, we use the is_home from odds. Simpler: use the opponent field convention.
        # In game logs, if opponent has "@" prefix it's a home game for the team.
        opp_a = g.get("opp_abbr", "")
        # Without @ convention, we can't perfectly tell, so use all games and compute
        # the spread between this team's avg and the league avg as a proxy.
        pass
    # Simpler approach: average scored in last 10 home games vs last 10 away games
    # Since we don't have is_home in team_scores, approximate with: for the current game,
    # the team is home/away. Compute the difference between their avg score and their
    # opponent's average allowed.
    # Best available: team_season_avg - opp_season_allowed_avg (captures matchup asymmetry)
    team_season_avg = season_avg
    opp_season_allowed_avg = opp_season_allowed
    home_vs_away_spread = team_season_avg - opp_season_allowed_avg

    # --- h2h_recent_record: win rate in last 5 H2H matchups ---
    h2h_games = [g for g in past_games if g["opponent"] == opp_name_lower]
    if len(h2h_games) >= 1:
        last_5_h2h = h2h_games[-5:]
        h2h_wins = 0
        for g in last_5_h2h:
            opp_scored_h2h = team_date_scored_lookup.get((opp_name_lower, g["game_date"]))
            if opp_scored_h2h is not None and g["scored"] > opp_scored_h2h:
                h2h_wins += 1
        h2h_recent_record = h2h_wins / len(last_5_h2h)
    else:
        h2h_recent_record = 0.5  # default: no H2H history

    # --- team_scoring_streak: consecutive games above/below scoring avg (signed) ---
    team_scoring_streak = _scoring_streak(past_games)

    # --- opp_scoring_streak: same for opponent ---
    opp_games = team_scores.get(opp_name_lower, [])
    opp_past_games = [g for g in opp_games if g["game_date"] < game_date]
    opp_scoring_streak = _scoring_streak(opp_past_games)

    # --- team_rest_advantage: (team rest days - opp rest days) ---
    opp_rest_days = 0
    if opp_past_games:
        opp_prev_date = opp_past_games[-1]["game_date"]
        opp_rest_days = min(date_diff(opp_prev_date, game_date), 14)
    team_rest_advantage = rest_days - opp_rest_days

    # --- team_season_win_pct: team's win % this season ---
    team_season_win_pct = _win_pct(team_name_lower, team_scores, team_date_scored_lookup, game_date)

    # --- opp_season_win_pct: opponent's win % this season ---
    opp_season_win_pct = _win_pct(opp_name_lower, team_scores, team_date_scored_lookup, game_date)

    # --- lineup_completeness: fraction of expected team minutes covered ---
    # Already computed above as lineup_completeness_val

    # --- team_stat_variance: variance in recent team scoring ---
    if len(last_10) >= 2:
        team_stat_variance = statistics.variance(last_10)
    else:
        team_stat_variance = 0.0

    # --- projected_vs_team_ewa: lineup_projected_stat / team_stat_ewa ---
    projected_vs_team_ewa = lineup_projected_stat / team_stat_ewa_val if team_stat_ewa_val > 0 else 1.0
    ...
    base_features = [
        ...
        team_season_win_pct,
        opp_season_win_pct,
        lineup_completeness_val,
        team_stat_variance,
        projected_vs_team_ewa,
    ]

    # --- H/I/J. MLB Pitcher Features (10 features, 0 for non-MLB) ---
    if league == "mlb" and player_position and starter_cache is not None:
        pitcher_feats = compute_pitcher_features(
            team_name_lower, opp_name_lower, game_date,
            team_game_players, logs_by_player, player_position,
            starter_cache, league_games_before,
        )
    else:
        pitcher_feats = [0.0] * 10

    # --- K. Park Factors (MLB only, uses home/away from game_odds) ---
...
def build_dataset(game_odds_events, team_scores, team_allowed, team_game_players,
    ...
    # Build a sorted list of all game dates for efficiency
        ...
        if spread_line is not None and spread_home_price is not None and spread_away_price is not None:
            ...
            if margin == 0:
                ...
                feats = compute_team_features(
                    ...
                    team_scores, team_allowed, team_game_players, logs_by_player,
                    league_games_before, stat_col, team_date_scored,
...
# ---------------------------------------------------------------------------
# 5-Fold CV Permutation Importance Pruning (Key F2 Innovation)
# ---------------------------------------------------------------------------
def cv_permutation_prune(X_train, y_train, feature_names, n_splits=5, threshold=PERM_IMPORTANCE_THRESHOLD):
    """Run 5-fold CV, compute permutation importance on each fold's validation set,
    average across folds, and return a boolean mask of features to keep.

    Features with mean permutation importance < threshold are dropped.
    """
    n_pos = int(np.sum(y_train == 1))
    n_neg = int(np.sum(y_train == 0))
    actual_splits = min(n_splits, max(2, n_pos, n_neg))

    if actual_splits < 2 or len(y_train) < actual_splits * 2:
        # Not enough data for CV — keep all features
        return np.ones(len(feature_names), dtype=bool), feature_names

    cv = StratifiedKFold(n_splits=min(5, actual_splits), shuffle=True, random_state=42)
    ...
    try:
        for fold_train_idx, fold_val_idx in cv.split(X_train, y_train):
            ...
            result = permutation_importance(
                fold_model, X_train[fold_val_idx], y_train[fold_val_idx],
    ...
    perm_importances /= n_folds_used

    # Keep features whose mean permutation importance >= threshold
    keep_mask = perm_importances >= threshold
    # Always keep at least 5 features (top-5 by importance if too many pruned)
    if np.sum(keep_mask) < 5:
        top_indices = np.argsort(perm_importances)[-5:]
        keep_mask = np.zeros(len(feature_names), dtype=bool)
        keep_mask[top_indices] = True

    kept = [f for f, k in zip(feature_names, keep_mask) if k]
    dropped = [f for f, k in zip(feature_names, keep_mask) if not k]
    if dropped:
        print(f"      Pruned {len(dropped)} features (perm imp < {threshold}): {dropped}")

    return keep_mask, kept
...
def run(league):
    print(f"\n{'=' * 60}")
    print(f"TEAM MODEL F2 — {league.upper()} (Walk-Forward)")
    print(f"{'=' * 60}\n")
    ...
    for train_book in ["fanduel", "draftkings"]:
        ...
        print(f"\n{'=' * 60}")
        print(f"TEAM MODEL F2 — {league.upper()} — Train on {train_book.upper()}")
        print(f"{'=' * 60}\n")
        ...
        for wi, window in enumerate(windows):
            ...
            # --- Per-market training with CV permutation importance pruning ---
            ...
            for mkt in all_markets:
                ...
                # F2 innovation: 5-fold CV permutation importance pruning
                print(f"    {mkt}: running 5-fold CV permutation importance pruning...")
                keep_mask, kept_features = cv_permutation_prune(
                    X_train_full, y_tr, FEATURES, n_splits=5, threshold=PERM_IMPORTANCE_THRESHOLD,
                )
                ...
                print(f"    {mkt}: train={len(mkt_train)} test={len(mkt_test)} "
                      f"acc={mkt_acc:.4f} auc={mkt_auc:.4f} kept={len(kept_features)}/{len(FEATURES)} dropped={dropped_n}")
            ...
            window_bets = make_team_bets(test_for_bets, y_proba, "team_f2", season)
        ...
        # --- Summary ---
        if not all_bets:
            print(f"No bets from any window for {train_book}!")
            continue
        ...
        print(f"\n{'=' * 60}")
        print(f"COMBINED WALK-FORWARD RESULTS ({train_book.upper()})")
        print(f"{'=' * 60}")
        ...
        out = f"/workspace/back-in-play/data/team_f2_{league}_{train_book[:2]}.json"
        ...
        try:
            league_key = f"{league}_team_f2_{train_book[:2]}"
            row = {
                "league": league_key,
        ...
        # Save model pickle
        if last_model:
            pkl_path = f"/workspace/back-in-play/data/team_f2_{league}_{train_book[:2]}.pkl"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Team Model F2: team-level betting predictions with CV permutation pruning")
