    ...
    for season_year in range(args.start_season, args.end_season + 1):
        ...
        for row in rows:
            ...
            db_row = {
                ...
                "game_date": game_date,
                "opponent": opponent,
                "started": safe_int(row.get("starts", 0)) > 0,
                "minutes": round(minutes, 1),
                "stat_goals": goals,
                "stat_assists": assists,
                # NOTE: clean_sheets is used in composite calc above but has no
                # dedicated column — stat_sog is shots-on-target, not available in FPL CSV
