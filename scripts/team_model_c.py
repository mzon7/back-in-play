#!/usr/bin/env python3
"""
Team Model C: Team-level betting predictions (spreads, totals, team totals).

Uses LightGBM with CV-based feature pruning (Model E architecture).
Derives team scores from player game logs (since game_odds.home_score is unpopulated).
...
Usage:
  python3 team_model_c.py --league nba
"""
...
FEATURES = [
    ...
    "team_home_away_ewa",
    "team_home_away_diff",
    # G. Starting Pitcher (MLB only, 0 for other leagues)
    "sp_k_per_ip_ewa",
    "sp_ip_ewa",
    "sp_season_k_per_ip",
    "sp_ip_consistency",
...
def compute_team_features(
    ...
    """Compute the 26 features for a single team in a single game (16 base + 10 pitcher).
    ...
    if not prev_player_logs:
        ...
        lineup_proj_last_game_mins = team_stat_ewa
        lineup_proj_10g_mins = team_stat_ewa
    else:
        lineup_proj_sum = 0.0
        lineup_proj_lgm = 0.0
        lineup_proj_10g = 0.0
        ...
        lineup_proj_last_game_mins = lineup_proj_lgm if lineup_proj_lgm > 0 else team_stat_ewa
        lineup_proj_10g_mins = lineup_proj_10g if lineup_proj_10g > 0 else team_stat_ewa
    ...
    # --- G/H/I. MLB Pitcher Features (10 features, 0 for non-MLB) ---
    ...
    # --- J. Park Factors (MLB only, uses home/away from game_odds) ---
...
def run(league):
    print(f"\n{'=' * 60}")
    print(f"TEAM MODEL C — {league.upper()} (Walk-Forward)")
    print(f"{'=' * 60}\n")
    ...
    for train_book in ["fanduel", "draftkings"]:
        ...
        print(f"\n{'=' * 60}")
        print(f"TEAM MODEL C — {league.upper()} — Train on {train_book.upper()}")
        print(f"{'=' * 60}\n")
        ...
        for wi, window in enumerate(windows):
            ...
            window_bets = make_team_bets(test_for_bets, y_proba, "team_c", season)
        ...
        out = f"/workspace/back-in-play/data/team_c_{league}_{train_book[:2]}.json"
        ...
        try:
            league_key = f"{league}_team_c_{train_book[:2]}"
            row = {
                "league": league_key,
    ...
    # Save model pickle
    if last_model:
        pkl_path = f"/workspace/back-in-play/data/team_c_{league}.pkl"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Team Model C: team-level betting predictions")
